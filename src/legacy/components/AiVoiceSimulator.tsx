...
const STATUS_BG:     Record<Status, string> = { pass: 'rgba(16,185,129,0.10)', warn: 'rgba(245,158,11,0.10)', fail: 'rgba(239,68,68,0.10)' };
const STATUS_BORDER: Record<Status, string> = { pass: 'rgba(16,185,129,0.22)', warn: 'rgba(245,158,11,0.22)', fail: 'rgba(239,68,68,0.22)' };

// ── Sub-components (defined outside to avoid remount on every render) ─────────

function WaveBars({ isPlaying, neon }: { isPlaying: boolean; neon: string }) {
  return (
        ...
        return (
          <div
            ...
            style={{
              ...
              background: `linear-gradient(to top, ${neon}, rgba(16,185,129,0.5))`,
              boxShadow: isPlaying ? `0 0 6px ${neon}80` : 'none',
...
function StatusIcon({ s }: { s: Status }) {
  if (s === 'pass') return <CheckCircle2 size={14} className="text-emerald-400 shrink-0" />;
  if (s === 'warn') return <AlertTriangle size={14} className="text-amber-400 shrink-0" />;
...
export default function AiVoiceSimulator({ previewMode }: { previewMode?: boolean }) {
  ...
  // Prevent concurrent API calls (guards against double-click after done state)
  ...
  // Stop speech when assistant changes
  useEffect(() => {
  ...
  const handlePlay = async () => {
    if (previewMode || isGenerating || callInFlight.current) return;
    ...
    // Guard: speechSynthesis not available (old browsers, some mobile WebViews)
    ...
    try {
      ...
      if (!hasTTS) {
        // No TTS support — just show the script as transcript
      ...
      // Progressive subtitle — word boundary events
  ...
  const btnLabel = isGenerating
    ? 'Generando script...'
    : isPlaying ? 'Detener'
    : done      ? 'Repetir simulación'
    : 'Escuchar simulación de voz';
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div className="glass-card rounded-2xl p-5 space-y-5">
          ...
          <div className="space-y-2.5">
            ...
            {inputFields.map(({ icon, ph, val, set }, i) => (
              <div key={i} className="relative">
                ...
                <input
                  ...
                  className="w-full rounded-xl pl-7 pr-3 py-3 text-xs text-slate-200 outline-none placeholder-slate-600 transition-colors focus:border-emerald-500/40"
            ...
            <div className="grid grid-cols-2 gap-2">
              {ASSISTANTS.map(a => (
                <button
                  ...
                  className="flex items-center gap-2 px-3 py-2.5 rounded-xl text-xs font-semibold transition-all duration-200 min-h-[44px]"
                  style={
                    assistantId === a.id
          ...
          <div>
            ...
            <div className="relative">
              <select
                ...
                className="w-full appearance-none rounded-xl px-3 py-3 text-xs text-slate-200 pr-8 outline-none"
        ...
        <div className="flex flex-col items-center justify-center">
          ...
          >
            ...
            {(['-left-1', '-right-1'] as const).map(side => (
            ...
            <div className="rounded-2xl overflow-hidden" style={{ background: 'linear-gradient(180deg,#060810 0%,#04060c 100%)', border: '1px solid rgba(255,255,255,0.06)' }}>
              ...
              <div className="px-5 py-5 space-y-4">
                ...
                >
                  <WaveBars isPlaying={isPlaying} neon={assistant.neon} />
                </div>
                ...
                <div className="flex justify-center">
                  <button
                    ...
                    className="flex items-center gap-2.5 px-6 py-3 rounded-full text-sm font-bold transition-all duration-300 disabled:opacity-50 min-h-[44px]"
                    style={{
        ...
        <div className="glass-card rounded-2xl p-5 space-y-4">
          ...
          <div className="space-y-2.5">
            {checklist.map((item, i) => (
              <div key={i} className="rounded-xl p-3 flex gap-3 items-start" style={{ background: STATUS_BG[item.status], border: `1px solid ${STATUS_BORDER[item.status]}` }}>
